 ...
 * v0.9.0-rc1
 */
angular.module('ngMaterial', ["ng","ngAnimate","ngAria","material.core","material.core.gestures","material.core.theming.palette","material.core.theming","material.components.autocomplete","material.components.backdrop","material.components.bottomSheet","material.components.button","material.components.card","material.components.checkbox","material.components.content","material.components.dialog","material.components.divider","material.components.chips","material.components.gridList","material.components.icon","material.components.input","material.components.list","material.components.modeless","material.components.popover","material.components.progressCircular","material.components.progressLinear","material.components.radioButton","material.components.select","material.components.sidenav","material.components.slider","material.components.sticky","material.components.subheader","material.components.swipe","material.components.switch","material.components.tabs","material.components.toast","material.components.tooltip","material.components.toolbar","material.components.whiteframe"]);
/*!
 * Angular Material Design
 * https://github.com/angular/material
 * @license MIT
...
/**
 * @ngdoc module
 * @name material.components.modeless
 */
angular.module('material.components.modeless', [
  'material.core',
  'material.components.backdrop'
])
  .directive('mdModeless', MdModelessDirective)
  .provider('$mdModeless', MdModelessProvider);
...
MdModelessDirective.$inject = ["$$rAF", "$mdTheming"];

/**
 * @ngdoc service
 * @name $mdModeless
 * @module material.components.modeless
 *
 * @description
 * `$mdModeless` opens a modeless over the app to inform users about critical information or require
 *  them to make decisions. There are two approaches for setup: a simple promise API
 *  and regular object syntax.
 *
 * ## Restrictions
 *
 * - The modeless is always given an isolate scope.
 * - The modeless's template must have an outer `<md-modeless>` element.
 *   Inside, use an `<md-content>` element for the modeless's content, and use
 *   an element with class `md-actions` for the modeless's actions.
 * - Modelesss must cover the entire application to keep interactions inside of them.
 * Use the `parent` option to change where modelesss are appended.
 *
 * ## Sizing
 * - Complex modelesss can be sized with `flex="percentage"`, i.e. `flex="66"`.
 * - Default max-width is 80% of the `rootElement` or `parent`.
 *
 * @usage
 * <hljs lang="html">
 ...
 *     Employee Alert!
 *   </md-button>
 *   <md-button ng-click="showModeless($event)" class="md-raised">
 *     Custom Modeless
 *   </md-button>
 *   <md-button ng-click="closeAlert()" ng-disabled="!hasAlert()" class="md-raised">
 *     Close Alert
 ...
 *    .controller('AppCtrl', AppController);
 *
 *   function AppController($scope, $mdModeless) {
 *     var alert;
 *     $scope.showAlert = showAlert;
 *     $scope.showModeless = showModeless;
 *     $scope.items = [1, 2, 3];
 *
 *     // Internal method
 *     function showAlert() {
 *       alert = $mdModeless.alert({
 *         title: 'Attention',
 *         content: 'This is an example of how easy modelesss can be!',
 *         ok: 'Close'
 *       });
 *
 *       $mdModeless
 ...
 *     }
 *
 *     function showModeless($event) {
 *        var parentEl = angular.element(document.body);
 *        $mdModeless.show({
 *          parent: parentEl,
 *          targetEvent: $event,
 *          template:
 *            '<md-modeless aria-label="List modeless">' +
 *            '  <md-content>'+
 *            '    <md-list>'+
 *            '      <md-item ng-repeat="item in items">'+
 *            '       <p>Number {{item}}</p>' +
 ...
 *            '  </md-content>' +
 *            '  <div class="md-actions">' +
 *            '    <md-button ng-click="closeModeless()">' +
 *            '      Close Modeless' +
 *            '    </md-button>' +
 *            '  </div>' +
 *            '</md-modeless>',
 ...
 *            items: $scope.items
 *          },
 *          controller: ModelessController
 *       });
 *       function ModelessController(scope, $mdModeless, items) {
 *         scope.items = items;
 *         scope.closeModeless = function() {
 *           $mdModeless.hide();
 *         }
 *       }
 *     }
 ...
 * </hljs>
 *
 * ### JavaScript: promise API syntax, custom modeless template
 * <hljs lang="js">
 * (function(angular, undefined){
 *   "use strict";
 *
 ...
 *     .controller('GreetingController', GreetingController);
 *
 *   // Fictitious Employee Editor to show how to use simple and complex modelesss.
 *
 *   function EmployeeEditor($scope, $mdModeless) {
 *     var alert;
 *
 *     $scope.showAlert = showAlert;
 *     $scope.closeAlert = closeAlert;
 ...
 *     $scope.userName = $scope.userName || 'Bobby';
 *
 *     // Modeless #1 - Show simple alert modeless and cache
 *     // reference to modeless instance
 *
 *     function showAlert() {
 *       alert = $mdModeless.alert()
 *         .title('Attention, ' + $scope.userName)
 ...
 *     }
 *
 *     // Close the specified modeless instance and resolve with 'finished' flag
 *     // Normally this is not needed, just use '$mdModeless.hide()' to close
 *     // the most recent modeless popup.
 *
 *     function closeAlert() {
 *       $mdModeless.hide( alert, "finished" );
 *       alert = undefined;
 *     }
 *
 *     // Modeless #2 - Demonstrate more complex modelesss construction and popup.
 *
 *     function showCustomGreeting($event) {
 *         $mdModeless.show({
 *           targetEvent: $event,
 ...
 *   }
 *
 *   // Greeting controller used with the more complex 'showCustomGreeting()' custom modeless
 *
 *   function GreetingController($scope, $mdModeless, employee) {
 *     // Assigned from construction <code>locals</code> options...
 *     $scope.employee = employee;
 *
 *     $scope.closeModeless = function() {
 *       // Easily hides most recent modeless shown...
 *       // no specific instance reference is needed.
 *       $mdModeless.hide();
 *     };
 *   }
 ...
 */
 ...
 /**
 * @ngdoc method
 * @name $mdModeless#alert
 *
 * @description
 * Builds a preconfigured modeless with the specified message.
 *
 * @returns {obj} an `$mdModelessPreset` with the chainable configuration methods:
 *
 * - $mdModelessPreset#title(string) - sets title to string
 * - $mdModelessPreset#content(string) - sets content / message to string
 * - $mdModelessPreset#ok(string) - sets okay button text to string
 * - $mdModelessPreset#theme(string) - sets the theme of the modeless
 *
 */

 /**
 * @ngdoc method
 * @name $mdModeless#confirm
 *
 * @description
 * Builds a preconfigured modeless with the specified message. You can call show and the promise returned
 * will be resolved only if the user clicks the confirm action on the modeless.
 *
 * @returns {obj} an `$mdModelessPreset` with the chainable configuration methods:
 *
 * Additionally, it supports the following methods:
 ...
 * - $mdModelessPreset#content(string) - sets content / message to string
 * - $mdModelessPreset#ok(string) - sets okay button text to string
 * - $mdModelessPreset#cancel(string) - sets cancel button text to string
 * - $mdModelessPreset#theme(string) - sets the theme of the modeless
 *
 */
...
/**
 * @ngdoc method
 * @name $mdModeless#show
 *
 * @description
 * Show a modeless with the specified options.
 *
 * @param {object} optionsOrPreset Either provide an `$mdModelessPreset` returned from `alert()`, and
 * `confirm()`, or an options object with the following properties:
 *   - `templateUrl` - `{string=}`: The url of a template that will be used as the content
 *   of the modeless.
 *   - `template` - `{string=}`: Same as templateUrl, except this is an actual template string.
 ...
 *   - `scope` - `{object=}`: the scope to link the template / controller to. If none is specified,
 *     it will create a new isolate scope.
 *     This scope will be destroyed when the modeless is removed unless `preserveScope` is set to true.
 *   - `preserveScope` - `{boolean=}`: whether to preserve the scope when the element is removed. Default is false
 *   - `disableParentScroll` - `{boolean=}`: Whether to disable scrolling while the modeless is open.
 *     Default true.
 *   - `hasBackdrop` - `{boolean=}`: Whether there should be an opaque backdrop behind the modeless.
 *     Default true.
 *   - `clickOutsideToClose` - `{boolean=}`: Whether the user can click outside the modeless to
 *     close it. Default false.
 *   - `escapeToClose` - `{boolean=}`: Whether the user can press escape to close the modeless.
 *     Default true.
 *   - `focusOnOpen` - `{boolean=}`: An option to override focus behavior on open. Only disable if
 *     focusing some other way, as focus management is required for modelesss to be accessible.
 *     Defaults to true.
 *   - `controller` - `{string=}`: The controller to associate with the modeless. The controller
 *     will be injected with the local `$mdModeless`, which passes along a scope for the modeless.
 *   - `locals` - `{object=}`: An object containing key/value pairs. The keys will be used as names
 *     of values to inject into the controller. For example, `locals: {three: 3}` would inject
 *     `three` into the controller, with the value 3. If `bindToController` is true, they will be
 *     copied to the controller instead.
 ...
 *     These values will not be available until after initialization.
 *   - `resolve` - `{object=}`: Similar to locals, except it takes promises as values, and the
 *     modeless will not open until all of the promises resolve.
 *   - `controllerAs` - `{string=}`: An alias to assign the controller to on the scope.
 *   - `parent` - `{element=}`: The element to append the modeless to. Defaults to appending
 *     to the root element of the application.
 *   - `onComplete` `{function=}`: Callback function used to announce when the show() action is
 *     finished.
 *
 * @returns {promise} A promise that can be resolved with `$mdModeless.hide()` or
 * rejected with `$mdModeless.cancel()`.
 */

/**
 * @ngdoc method
 * @name $mdModeless#hide
 *
 * @description
 * Hide an existing modeless and resolve the promise returned from `$mdModeless.show()`.
 *
 * @param {*=} response An argument for the resolved promise.
 */
...
/**
 * @ngdoc method
 * @name $mdModeless#cancel
 *
 * @description
 * Hide an existing modeless and reject the promise returned from `$mdModeless.show()`.
 *
 * @param {*=} response An argument for the rejected promise.
 */
...
function MdModelessProvider($$interimElementProvider) {
  ...
  advancedModelessOptions.$inject = ["$mdModeless", "$mdTheming"];
  modelessDefaultOptions.$inject = ["$mdAria", "$document", "$mdUtil", "$mdConstant", "$mdTheming", "$mdModeless", "$timeout", "$rootElement", "$animate", "$$rAF", "$q"];
  ...
  function modelessDefaultOptions($mdAria, $document, $mdUtil, $mdConstant, $mdTheming, $mdModeless, $timeout, $rootElement, $animate, $$rAF, $q) {
    return {
      ...
      disableParentScroll: true,
      transformTemplate: function(template) {
      	return '<div class="md-modeless-container">' + template + '</div>';
      }
    };
...
MdModelessProvider.$inject = ["$$interimElementProvider"];
...
function MdPopoverDirective($timeout, $window, $$rAF, $document, $mdUtil,
  ...
  function postLink(scope, element, attr, contentCtrl) {
    ...
    scope.$on('$destroy', function () {
      ...
      $rootElement.off('keyup', rootElementKeyupCallback);
    });
...
MdPopoverDirective.$inject = ["$timeout", "$window", "$$rAF", "$document", "$mdUtil", "$mdTheming", "$rootElement", "$mdAria", "$mdConstant"];
})(); 
